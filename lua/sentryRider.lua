...
-- Player flag constants (from Source engine documentation)
local FL_DUCKING = 2 -- Player is ducking/crouched
local FL_ONGROUND = 1 -- Player is on ground

-- Input button constants (from Source engine documentation)
local IN_JUMP = 2 -- Jump button bitmask
...
-- Air movement prediction constants
local AIR_ACCEL = 10 -- Default sv_airaccelerate value
local AIR_CAP = 30 -- Air speed cap (units/sec added per tick in air)
local SWING_DETECTION_THRESHOLD = 50 -- Minimum Z velocity to consider "swinging"
local GROUND_SPEED_CAP_FACTOR = 0.8 -- When landing, speed gets capped to this fraction of max

--[[
    SWING/AIR MOVEMENT PREDICTION SYSTEM
    -----------------------------------
    TF2 movement physics:
    - Ground: Full acceleration control, speed capped to class max
    - Air: Limited acceleration (sv_airaccelerate), no speed cap while airborne
    - Landing: Speed gets capped to ground max, losing excess velocity
    
    This system predicts:
    1. When player will leave ground (swing initiation)
    2. Air trajectory and landing position
    3. Speed cap effects when landing
    4. Whether target will be reachable after swing completes
]]

-- Predicts if player is about to leave ground based on velocity and input
local function PredictSwingInitiation(player, cmd)
	if not player or not cmd then
		return false, 0
	end
	...
	if not onGround then
		return true, 999 -- Already swinging
	end

	-- Check for jump input
	local buttons = cmd:GetButtons()
	local jumping = (buttons & IN_JUMP) ~= 0
	...
	-- Predict swing if:
	-- 1. Player is jumping, OR
	-- 2. Already has significant upward velocity
	local willSwing = jumping or (zVel > SWING_DETECTION_THRESHOLD)

	-- Estimate time until landing (simple ballistic prediction)
	local timeToLand = 0
	if willSwing and zVel > 0 then
		-- Approximate time to peak + time to fall back to current height
		local gravity = 800 -- TF2 gravity (units/sec²)
		local timeToPeak = zVel / gravity
		timeToLand = timeToPeak * 2 -- Up and back down
	end

	return willSwing, timeToLand
end

-- Predicts where player will land after swing, accounting for speed cap
local function PredictSwingLanding(player, timeToLand)
	if not player or timeToLand <= 0 then
	...
	-- Predict horizontal landing position (air movement has minimal control)
	local landingPos = Vector3(
		pos.x + vel.x * timeToLand,
		pos.y + vel.y * timeToLand,
		pos.z -- Assume same height for simplicity
	)

	-- Predict speed cap effect when landing
	local horizontalSpeed = math.sqrt(vel.x ^ 2 + vel.y ^ 2)
	local maxGroundSpeed = GetMaxAllowedSpeed(player)
	local cappedSpeed = math.min(horizontalSpeed, maxGroundSpeed * GROUND_SPEED_CAP_FACTOR)

	return landingPos, cappedSpeed
end

-- Checks if target will be reachable after swing completes
local function CanReachTargetAfterSwing(player, target, landingPos, cappedSpeed, timeToLand)
	if not landingPos or not target then
		return false
	end

	-- Distance from predicted landing to target
	local landingToTarget = target - landingPos
	landingToTarget.z = 0 -- Only horizontal distance matters
	local distanceToTarget = landingToTarget:Length()

	-- Estimate if we can reach target with capped ground speed
	local maxGroundSpeed = GetMaxAllowedSpeed(player)
	local timeToReachFromLanding = distanceToTarget / math.max(cappedSpeed, maxGroundSpeed * 0.5)

	-- Consider it reachable if we can get there in reasonable time
	local totalTime = timeToLand + timeToReachFromLanding
	return totalTime < 3.0 -- Max 3 seconds total to be considered viable
end

--[[
    ENHANCED MOVEMENT PREDICTION
    ---------------------------
    Combines existing ground prediction with swing prediction for complete
    movement analysis. Decides whether to:
    1. Continue normal ground movement
    2. Wait for swing to complete
    3. Abort current movement attempt
]]
local function PredictCompleteMovement(player, cmd, target)
	if not player or not cmd or not target then
	...
	-- Check swing status
	local isSwinging, timeToLand = PredictSwingInitiation(player, cmd)

	if isSwinging then
		-- Player is swinging - predict landing
		local landingPos, cappedSpeed = PredictSwingLanding(player, timeToLand)
		local canReach = CanReachTargetAfterSwing(player, target, landingPos, cappedSpeed, timeToLand)
		...
		return {
			...
			canReachTarget = canReach,
			shouldWait = not canReach or timeToLand > 0.5,
		},
			"swing_predicted"
	else
		-- Normal ground movement prediction
		local tickInt = globals.TickInterval()
		local predictedPos = PredictNextTickPosHorizontal(player, tickInt)
