...
local Menu = {
    ...
    tags = true,
    partyCallaut = true,
    Enable_chokecheck = false,
    Enable_bhopcheck = true,
    Enable_Aimbotcheck = true
}
...
local function CheckChoke(player, entity)
    ...
    local oldsimtimeticks = Conversion.Time_to_Ticks(oldSimTime)
    local simtimeticks = Conversion.Time_to_Ticks(simTime)
    print(delta) 
    print(oldSimTime)
    print(simTime)
    ...
    elseif deltaTicks < 1 then
        StrikePlayer("Warp/DT", player)
        return true
    else
...
local function OnCreateMove(userCmd)
    ...
    if Menu.Enable_Aimbotcheck and CheckAimbot() == true then goto Aimbot end  --detect silent aimbot users
    ...
    for i = 1, #players do
        ...
        if Menu.Enable_bhopcheck and CheckBhop(player, currentData, entity) == true then break end --detects rage Bhop

        --local XconnectionState = entities.GetPlayerResources():GetPropDataTableInt("m_iConnectionState")[idx]
        if Menu.Enable_chokecheck and prevData then
...
local function doDraw()
    ...
    if input.IsButtonDown(KEY_INSERT) then  -- Replace 72 with the actual key code for the button you want to use
        ...
        if Lbox_Menu_Open == true and ImMenu.Begin("Cheater Detection", true) then
            ...
            ImMenu.BeginFrame(1)
            Menu.Enable_chokecheck = ImMenu.Checkbox("choke detect", Menu.Enable_chokecheck)
            if Menu.Enable_chokecheck == true then
            ...
            -- Enable_bhopcheck
            ImMenu.BeginFrame(1)
            Menu.Enable_bhopcheck = ImMenu.Checkbox("bhop detect", Menu.Enable_bhopcheck)
            if Menu.Enable_bhopcheck == true then
            ...
            -- Aimbot FOV Slider
            ImMenu.BeginFrame(1)
            Menu.Enable_Aimbotcheck = ImMenu.Checkbox("aimbot detect", Menu.Enable_Aimbotcheck)
            if Menu.Enable_Aimbotcheck == true then
