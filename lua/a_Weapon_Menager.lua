...
local mcriticalhealthesp = menu:AddComponent(MenuLib.Slider("priority below HP%",    0, 100, 0))
...
local msandwitchex      = menu:AddComponent(MenuLib.Checkbox("inffinite sandwich exploid", false))                      -- Infinite Respawn Timer                         -- Retry When Low HP
local mRetryLowHPValue  = menu:AddComponent(MenuLib.Slider("HP% Retry ",  0, 100, 0))                    -- Retry When Low HP Value                         -- Auto Fake Latency
local mAutoFLDist       = menu:AddComponent(MenuLib.Slider("AFL Activation Distance",    0, 700, 0))                -- Auto Fake Latency Distance (530 is based on two players walking towards each other)
...
local mRandPingValue    = menu:AddComponent(MenuLib.Slider("Ping Randomness",          0, 15, 0))                       -- Random Ping Value                         -- Random Fakelag
local mRandLagValue     = menu:AddComponent(MenuLib.Slider("Fakelag Randomness",       0, 200, 0))                     -- Random Fakelag Value
...
--[[
    ...
    if mRandLagValue:GetValue() >= 1 then                                                                    -- If Random Fakelag is enabled
...
if sneakyboy then goto continue end
        ...
        for i, vPlayer in pairs(players) do  -- For each player in the game
            ...
            if not closestPlayer then break end
         -- Swing Prediction
        if Swingpred:GetValue() and distance < 500 then
            ...
            if not mWswitchoptions:IsSelected("Self Defence") and not myteam then
                meleedist = 0
            end
            --[[Swing rediction                     ]]
            -- Check if estimated hit time is within range, enemy is not on the same team, and melee distance
            if mWswitchoptions:IsSelected("Self Defence") and meleedist and not myteam then
                -- Set attack button
                if estime > 0 and estime <= 0.25 or swingdist and (pWeapon:IsMeleeWeapon() == true)  then
       ...
       -- print( clip )
        ...
        if mWswitchoptions:IsSelected("Auto_Combo") then
            if PlayerClass == 5 then
                -- medic class
                cond_Melee = not myteam and meleedist and mWswitchoptions:IsSelected("Self Defence") and (pWeapon:IsMeleeWeapon() == false)
        ...
        if estime and estime > 0 and estime <= 0.25 and cond_Melee or meleedist and cond_Melee and (pWeapon:IsMeleeWeapon() == true) then
            -- Set attack button
...
if procent > 0 then
    ...
    for i, p in ipairs( players ) do
        ...
        if mAutoFLDist:GetValue() >= 1 and (pWeapon:IsMeleeWeapon() == true)            -- If Auto Fake Latency is enabled, and we are using aoWeaponmAutoweapon weapon, and we are not invisible
