...
-- Helper function to adjust pitch using modulo for constant time
local function ClampPitch(pitch)
    pitch = pitch % 360
    return (pitch > 180) and (pitch - 360) or pitch
end

-- Helper function to clamp angles within -180 to 180 using modulo for constant time
local function ClampAngle(angle)
    return ((angle + 180) % 360) - 180
end

-- Function to convert a vector to Euler angles, with clamped pitch and yaw
local function VectorToEulerAngles(vector)
    -- Convert the vector to Euler angles
    local angles = vector:Angles()

    -- Adjust and clamp pitch
    local pitch = ClampAngle(ClampPitch(angles.x))

    -- Clamp yaw
    local yaw = ClampAngle(angles.y)
...
local function HandleArcTrajectory(results, vStartPosition, vStartAngle, vVelocity, vCollisionMin, vCollisionMax, fGravity, fDrag)
...
local function TraceAndSimulateTrajectory(pLocal, pWeapon)
    ...
    elseif iItemDefinitionType > 3 then
        traceResults = HandleArcTrajectory(traceResults, vStartPosition, vStartAngle, vVelocity, vCollisionMin, vCollisionMax, fGravity, fDrag)
    else
