...
local function GetEntityHitboxes(pEntity)
	...
	local setHitboxes = studiomodel:GetHitboxSet(pEntity:GetPropInt("m_nHitboxSet") or 0)
...
callbacks.Register("Draw", function()
	...
	for i = 0, entities.GetHighestEntityIndex() do
		...
		if pEntity and pEn ~= pLocalPlayer then
			for _, aVerts in pairs(GetEntityHitboxes(pEntity)) do
				COLOR(255, 0, 255, 255)
				if #aVerts == 8 then
