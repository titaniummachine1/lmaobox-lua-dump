...
function ImMenu.GetInteraction(x, y, width, height, id)
    ...
    -- Is a popup active?
    if ImMenu.ActivePopup ~= nil and not ImMenu.inPopup then
        return false, false, false
    end

    local mouseX, mouseY = input.GetMousePos()
    local hovered = (mouseX >= x and mouseX <= x + width and mouseY >= y and mouseY <= y + height) or id == ImMenu.ActiveItem
    local clicked = hovered and (input.IsButtonPressed(MOUSE_LEFT) or input.IsButtonPressed(KEY_ENTER))
    local active = hovered and (input.IsButtonDown(MOUSE_LEFT) or input.IsButtonDown(KEY_ENTER))
