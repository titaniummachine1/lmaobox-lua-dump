...
local mAutoWeaponDist   = menu:AddComponent(MenuLib.Slider("melee Switch Distance",    0, 400, 77))                 -- AutooWeaponmAutoweapon Switch Distances
    ...
    --[[----------------------- WEAPON MANAGER -----------------------------------------]]
        ...
        if mAutoweapon:GetValue() == false then goto continue end
            ...
            if not vPlayer:IsValid() and (distance >= 777) then goto continue end
                ...
                --check if can use mele without ricking loosing friend 
                --if mWswitchclasses:IsSelected("Medic") and PlayerClass == 7 then

                --[[MODE PICKER]]-- it gets confuzing fast letme make goto.
