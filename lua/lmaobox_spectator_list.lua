...
alib.settings.font = draw.CreateFont(
	"Arial",
	16,
	1000,
	E_FontFlag.FONTFLAG_ANTIALIAS | E_FontFlag.FONTFLAG_CUSTOM | E_FontFlag.FONTFLAG_GAUSSIANBLUR
)
	...
	if stage == E_ClientFrameStage.FRAME_NET_UPDATE_POSTDATAUPDATE_END then
		...
		for _, player in pairs(players) do
			if
				...
				and not player:IsAlive()
				and player ~= localplayer
				and player:GetTeamNumber() == localplayer:GetTeamNumber()
			then
