...
local function CalcStrafe()
    ...
    for idx, entity in ipairs(players) do
        ...
        -- Get velocity and check conditions
        ...
        if entity == pLocal then
            ...
            if not onGround and autostrafe == 2 and #pastPositions >= maxPositions then
                ...
                v = v / (maxPositions - 1)
                print("Using calculated velocity from past positions:", v)
            else
                v = entity:EstimateAbsVelocity()
                print("Using estimated velocity:", v)
            end
        end

        -- Calculate the current angle from velocity
        local angle = v:Angles()
        print("Calculated angle for entity", entityIndex, ":", angle)

        if lastAngles[entityIndex] == nil then
            print("Setting initial last angle for entity", entityIndex)
        ...
        -- Set up vectors for calculating strafe angle
        local vector1 = Vector3(1, 0, 0)
        local vector2 = Vector3(1, 0, 0)

        -- Apply deviation based on deltas
        ...
        -- Calculate the distance between the vectors (inaccuracy measure)
        local distance = (vector1 - vector2):Length()
        inaccuracy[entityIndex] = distance

        -- Save the calculated strafe angle
        strafeAngles[entityIndex] = avgDelta

        -- Save the current delta for the next calculation
...
local function checkInRange(targetPos, spherePos, sphereRadius, pWeapon)
    ...
    if TotalSwingRange > distanceAlongVector then
...
SwingHullSize = 38 --38x38x38 -- or -Vector3(-18,-18,-18) by Vector3(18,18,18) for trace hull
SwingHalfhullSize = SwingHullSize / 2

if pWeaponDef:GetName() == "The Disciplinary Action" then
    SwingHullSize = 55.8 --55.8x55.8x55.8 -- or -Vector3(-55.8,-55.8,-55.8) by Vector3(55.8,55.8,55.8) for trace hull
    SwingHalfhullSize = SwingHullSize / 2
    swingrange = 81.6
end
...
-- Predict both players' positions after swing
    ...
    if pLocal:EstimateAbsVelocity() == 0 then
        ...
        print(strafeAngles[entities.GetLocalPlayer():GetIndex()])
        strafeAngle = Menu.Misc.strafePred and strafeAngles[entities.GetLocalPlayer():GetIndex()] or 0
