...
local function FireGameEvent(event)
	if event:GetName() == "player_death" then
		...
		if mode == modes.priority and playerlist.GetPriority(victim) < 1 then
			return
		end
		...
		if not attacker or not attacker:IsPlayer() or attacker:GetIndex() ~= localplayerIndex then
			return
		end
